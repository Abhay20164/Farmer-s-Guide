...
def lm():
        ...
        for i in rec:
            ...
            l1.append(i[1])
            c=c+1
        plt.pie(l,labels=l1,shadow="true",autopct='%1.1f%%',explode=(0.05,0.05,0.05,0.05))
        plt.show()
def grap():
    g=Frame(m)
    ...
    b1=Button(g,text="BACK",font=("fixedsys",15),fg="black",borderwidth=20,command=g.destroy,bg="LIGHTGREY",width=25)
    b1.grid(row=1)
    g.mainloop()
...
Label(m,text="    \t\tFARMERS GUIDE \t\t    ",font=("fixedsys",30),fg="white",bg="black").place(x=150,y=0)
cat=Frame(m)
